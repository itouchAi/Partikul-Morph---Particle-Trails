      ...
      {(audioMode !== 'none' && (!isUIHidden || musicShowInCleanMode) && !isDrawing) && (
          <div className="absolute top-6 left-1/2 -translate-x-1/2 z-[60] group/player pb-10">
              ...
              <div 
                  ...
                  ${isLightMode ? 'bg-white/20 border-white/40 text-black' : 'bg-black/20 border-white/10 text-white'}
                  overflow-hidden
                  `}
                           ...
                           `}>
                              <div className="w-full overflow-hidden whitespace-nowrap mask-linear-fade flex items-center justify-end">
                                  <div className={`${(audioTitle && audioTitle.length > 18) ? 'animate-marquee-loop' : ''} flex flex-row items-center`}>
                                     {/* FIX: Add pr-2 for more padding (8px) */}
                                     <span className="text-[10px] tracking-wide block pr-2" style={{ fontFamily: musicFont, fontWeight: musicBold ? 'bold' : 'normal', fontStyle: musicItalic ? 'italic' : 'normal', whiteSpace: 'nowrap' }}>{audioTitle || "Bilinmeyen Şarkı"}</span>
                                     {(audioTitle && audioTitle.length > 18) && (
                                         <span className="text-[10px] tracking-wide ml-6 block pr-2" style={{ fontFamily: musicFont, fontWeight: musicBold ? 'bold' : 'normal', fontStyle: musicItalic ? 'italic' : 'normal', whiteSpace: 'nowrap' }}>{audioTitle || "Bilinmeyen Şarkı"}</span>
                                     )}
                                  </div>
                              </div>
              ...
              {showMusicSettings && (
                  <div className="absolute top-20 left-1/2 -translate-x-1/2 w-64 bg-[#111]/95 backdrop-blur-xl border border-white/20 rounded-xl p-4 shadow-[0_10px_40px_rgba(0,0,0,0.8)] animate-config-pop z-20" onPointerDown={stopProp}>
                      ...
                      <div className="space-y-2">
                          ...
                          {/* MOOD SYNC TOGGLE - NEW */}
                          <div className="flex items-center justify-between bg-white/5 p-2 rounded-lg border border-white/5">
                              <div className="flex flex-col">
                                  <span className={`text-[10px] ${isMoodSyncActive ? 'text-blue-200' : 'text-gray-300'}`}>Mood-Sync</span>
                                  <span className="text-[8px] text-gray-500 italic">Duyguya Göre Değiş</span>
                              </div>
                              <button onClick={onToggleMoodSync} className={`w-8 h-4 rounded-full relative transition-colors ${isMoodSyncActive ? 'bg-blue-600' : 'bg-white/10'}`}><div className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white transition-transform ${isMoodSyncActive ? 'translate-x-4' : 'translate-x-0'}`} /></button>
                          </div>

                          {/* BLOOM (GLOW) TOGGLE - NEW */}
                          <div className="flex items-center justify-between bg-white/5 p-2 rounded-lg border border-white/5">
                              <span className={`text-[10px] ${enableBloom ? 'text-pink-300' : 'text-gray-300'}`}>Neon Glow (Bloom)</span>
                              <button onClick={onToggleBloom} className={`w-8 h-4 rounded-full relative transition-colors ${enableBloom ? 'bg-pink-600' : 'bg-white/10'}`}><div className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white transition-transform ${enableBloom ? 'translate-x-4' : 'translate-x-0'}`} /></button>
                          </div>
      ...
      {(showInfoPanel && audioMode !== 'none') && !isDrawing && (
          <div 
            ...
            style={{
                ...
                width: isInfoExpanded ? '500px' : '230px', 
                height: isInfoExpanded ? '500px' : '252px', 
              ...
              >
                  ...
                  <div className="pt-[65px] px-4 pb-4 flex flex-col items-center justify-start flex-grow">
                      <div className="text-center w-full mt-0 mb-2">
                          {/* FIX: Ensure truncated artist name doesn't overflow. Added max-w-[190px] and mx-auto */}
                          <h3 className={`font-bold leading-tight tracking-tight drop-shadow-sm text-xl w-full truncate max-w-[190px] mx-auto px-2 ${isLoadingInfo ? 'animate-pulse opacity-50' : ''}`}>{songInfo ? songInfo.artistName : "Analiz Bekleniyor..."}</h3>
                          
                          {/* FIX: Added truncation to BIO text */}
                          {songInfo && !isUnknownArtist && !isLoadingInfo && ( <p className="opacity-70 font-mono text-[10px] mt-1 truncate w-full max-w-[190px] mx-auto">{songInfo.artistBio}</p> )}
                      </div>
                      
                      {/* PREVIEW TEXT (Front Face) */}
                      {songInfo && !isLoadingInfo && ( 
                          <div className="w-full px-1 flex flex-col gap-2 mt-1 overflow-hidden h-[110px]">
                              {/* FIX: Ensure meaning text doesn't overflow container height */}
                              <div className={`text-xs leading-snug text-white font-medium italic text-center drop-shadow-md overflow-hidden text-ellipsis line-clamp-3`}>
                                  <span className="font-bold not-italic opacity-80 text-[10px] mr-1 block mb-0.5 text-blue-300">Şarkı Analizi (TR):</span>
                                  "{songInfo.meaningTR}"
                              </div>
                              <div className={`text-xs leading-snug text-white/90 font-medium italic text-center drop-shadow-md overflow-hidden text-ellipsis line-clamp-3`}>
                  ...
                  <div className="flex-1 p-8 pt-6 overflow-y-auto custom-thin-scrollbar relative z-10">
                      {songInfo ? ( 
                          <div className="space-y-8"> 
                              ...
                              {/* --- MOOD BADGE IN BACK PANEL --- */}
                              {songInfo.mood && (
                                  <div className="mt-4 pt-4 border-t border-white/10 flex items-center gap-2">
                                      <span className="text-[10px] font-mono uppercase opacity-60">Mood:</span>
                                      <span className="text-xs bg-blue-500/20 text-blue-300 px-2 py-1 rounded-full uppercase font-bold tracking-wide border border-blue-500/30">{songInfo.mood}</span>
                                  </div>
                              )}
                          </div> 
